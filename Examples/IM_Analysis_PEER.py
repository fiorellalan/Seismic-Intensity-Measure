...
filename = os.path.join("Output","IM_PEER")
...
for ll, doss in enumerate(dire):
 ...
 for asc in glob(doss):
    ...
    figname =os.path.join("FIGURES","FIG_IM_" + tr.stats.network +'_' + tr.hdr.evname + '_' + tr.stats.station + '_' + tr.stats.channel + ".pdf")
    print (figname)
    ...
    try:
        ...
        time,acc,vel,spo, fminu, fmaxu = tool.baseline(tr.data,time,const=(1/980),fmin=0.1,npoles=4,coswin=0.05) 
        #pass 
