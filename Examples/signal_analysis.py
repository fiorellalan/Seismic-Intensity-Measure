...
class mytools:
    ...
    def baseline (self,acc,time,**kwargs):
        ...
        # cut
        cut_beg = np.int32(np.ceil(cutmin/delta))
        cut_end = np.int32(np.floor(cutmax/delta))
    ...
    def cav (self, **kwargs):
        ...
        nw   =np.int32(np.floor(len(acc)*self.delta/t0));     # number of windows
        ns   =np.int32(np.ceil(t0/self.delta));               # npts in each time window

        scav = 0;
        scavtime  = np.zeros(int(nw*ns));
        for ii in range(1,nw):
            w    = [];
            imin =np.int32((ii-1)*ns+1);
            imax =np.int32(ii*ns+1);
            if imax < len(acc):
    ...
    def response (self, **kwargs):
        ...
        for j in range(0,npts_out):
            ...
            elle = 1.0
            if dp[j]<self.delta: 
                elle =np.int32(self.delta/dp[j]+1.0 - 0.00001)
            dtl = self.delta/elle
            omega = 2*np.pi/per[j]
            w2 = omega**2
            w2d = w2*dtl
