...
filename = os.path.join("Output","IM_Tohoku")
fout = open(filename, 'w+b')
Parameters = {}
cut = {}
cut['KiK-Net.MYGH10'] = [10,100]
cut['K-Net.FKS001'] = [0,90]
...
dire = [os.path.join("Input","Tohoku2021","*.*")]
for ll, doss in enumerate(dire):
 print (doss)
 for asc in glob(doss):
    tr = tool.read_knet(asc)
 ...
 for asc in glob(doss):
    ...
    figname =os.path.join("FIGURES","FIG_IM_" + tr.stats.network +'_'+ tr.stats.station + '_' + tr.stats.channel + "_RAW.pdf")
    print (figname)
    ...
    try:
        ...
        print(cutmin, cutmax)
        time,acc,vel,spo, fminu, fmaxu = tool.baseline(tr.data,time,cutmin=cut[cname][0],cutmax=cut[cname][1],const=1,fmin=0.1,npoles=4,coswin=0.05) 
    except KeyError:
        time,acc,vel,spo, fminu, fmaxu = tool.baseline(tr.data,time,const=1,fmin=0.1,npoles=4,coswin=0.05) 
        #pass 
